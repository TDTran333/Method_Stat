...
Based on the histogram of the simulated portfolio values, we notice that the porfolio of call options as a function of underlying asset price at t = 0 is right skewed. Call option prices are therefore more sensible to a rise in the price of the underlying asset rather than a decline.
...
```{r echo=FALSE}
# Output the results
cat(sprintf("Calls value and maturity usually have positive relationship, and we can observe that 5 days in less until maturity lower portfolio value by  %.2f$.\n", abs(M_choc)))
```

Summarization of P&N distribution and it's drivers

1. Portfolio price and underlying price have positive relationship
2. P&N is right skewed because of the relationship between Calls value and underlying asset price at t = 0
...
Compared once again to the previous models, the VAR and ES are similar to the second model. Which is a bit surprising because model 2 doesn't take in account the dependance between the two sources of risks while model 5 does; the asset value depend in a way on the conditionel variance. 
...
Here is a table summarizing results of each models. Let compare the models:

Models 1 and 4 is the most conservative
Models 2 and 5 are somewhat less conservative than models 1 and 4
Model 3 is the most agressive of the 5 models invastigated

Model 1 and 4 are those who doesn't take in account a negative dependance and therefore they are most conservators
While model 3 take in account a negative dependance, and produce a more aggresive model
Model 2 and 5 are in between
