...
hist(sim_ret_1, nclass = round(10 * log(length(sim_ret_1))), probability = TRUE, main = "Histogram of 10 000 simulated return", xlab = "simulated returns")
```
...
hist(sim_S_1, nclass = round(10 * log(length(sim_S_1))), probability = TRUE, main = "Histogram of 10 000 simulated prices", xlab = "simulated prices")

# Add a verticale line that represent last price asset
abline(v   = S0,
       lty = 1,
       lwd = 2.5,
       col = "blue")
...
Based on the previous histogram (and the simulation) we can notice that the porfolio of call in function of the price of the underlying asset at t0 is right skewed. Calls value is therefore more sensible to a rising price of the underlying asset than a falling one.
...
U_2 <- pstd(log_return, mean = theta_vol_3[1], sd = theta_vol_3[2], nu = theta_vol_3[3])
...
Re-price the portfolio in one week assuming the same parametric model but shifted by the one-year
ATM implied volatility difference. Note. ATM means at-the-money, which means that m = 1. Assume that the one-year ATM implied volatility
given by the VIX is (a1 + a4).

```{r}

vix_1y_atm_IV <- theta_vol[1] + theta_vol[4]
vix_atm_IV <- vol_surface(S0, S0, M - t/250, theta_vol[1], theta_vol[2], theta_vol[3], theta_vol[4])

vol_shift <- vix_1y_atm_IV - vix_atm_IV 
...
colnames(Results) <- c("M1: 1 risk/Gauss", "M2: 2 risks/Gauss", "M3: 2 risk/Copula", "M4: Vol Surface", "M5: Full Approach")
rownames(Results) <- c("Value At Risk", "Expected Shortfall")
