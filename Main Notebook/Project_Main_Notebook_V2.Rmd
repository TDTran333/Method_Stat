...
colnames(book) <- c("Quantity", "Option Type", "Strike Price", "Maturity", "Option Value", "Position Value")
...
Note. ATM means at-the-money, which means that m = 1. Assume that the one-year ATM implied volatility
given by the VIX is (a1 + a4).

```{r}

vix_atm_IV <- theta_vol[1] + theta_vol[4]
vix_1y_atm_IV <- vol_surface(S0, S0, 1, theta_vol[1], theta_vol[2], theta_vol[3], theta_vol[4])
...
# Store in 'sim_ret_3' daily log returns for the underlying asset and the VIX
for (i in 1:t) {
  sim_ret_4[,,i] <- c(sim_log_ret[(H * (i - 1) + 1):(H * i)], sim_vix_ret[(H * (i - 1) + 1):(H * i)])
}
...
# Compute the underlying asset price and the VIX value one week from now
for (i in 1:H) {
  sim_S_4[i]   <- S0 * exp(sum(sim_ret_3[i,1,]))
  sim_vol_4[i] <- vol0 * exp(sum(sim_ret_3[i,2,]))
}
...
# Loop through H simulations and price each option
for (i in 1:H){
  sim_price_4[i,] <- mapply(price_BS, sim_S_3[i], K, rf_m_t, sim_vol_3[i], M - t / 250, Type)
}
