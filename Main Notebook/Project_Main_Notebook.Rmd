...
garch_returns    <- garch(x = log_return, 
                          order = c(1, 1),
                          series = NULL)

Resid_returns    <- garch_returns$residuals
Resid_returns    <- Resid_returns[-1]

# Residuals of the log-returns of the Vix 
ar1_vix    <- ar.ols(vix_return, 
                    order.max = 1, 
                    demean = F, 
                    intercept = T)

Resid_vix  <- ar1_vix$resid
Resid_vix  <- Resid_vix[-1]
