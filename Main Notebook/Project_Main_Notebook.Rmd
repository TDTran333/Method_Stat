...
```{r}
# Create matrix
book_1 <- matrix(data = NA, nrow = 4, ncol = 6)

# Assign names to columns
# Option Type: Call = 1, Put = 0
colnames(book_1) <- c("Quantity", "Option Type", "Strike Price", "Maturity", "Option Value", "Position Value")

# Store initial values
book_1[1,1:4] <- c(1, 1, 1600, 20 / 250)
book_1[2,1:4] <- c(1, 1, 1650, 20 / 250)
book_1[3,1:4] <- c(1, 1, 1750, 40 / 250)
book_1[4,1:4] <- c(1, 1, 1800, 40 / 250)
```
...
book_1[1,5] <- price_BS(S_1, K[1], r_1, vol_1, M[1], Type[1])
book_1[2,5] <- price_BS(S_1, K[2], r_1, vol_1, M[2], Type[1])
book_1[3,5] <- price_BS(S_1, K[3], r_2, vol_1, M[3], Type[1])
book_1[4,5] <- price_BS(S_1, K[4], r_2, vol_1, M[4], Type[1])

# Compute the price of the portfolio and store it in 'PF_price_1'
book_1[,6] = book_1[,1]*book_1[,5]

PF_price_1 <- sum(book_1[,6])
```
...
cat(sprintf("\nPortfolio detail:\n\n"))
print(format(book_1, digits=2), quote = FALSE)
```
...
nb_opts <- nrow(Market$calls) + nrow(Market$puts)
...
# Latest underlying asset price (spot price)
mkt_vol[,1] <- t(t(rep(sp_500[n_obs], nb_opts)))

# Strike price of call options
mkt_vol[1:nrow(Market$calls),2] <- Market$calls[,1]

# Strike price of put options
mkt_vol[(nrow(Market$calls)+1):nb_opts,2] <- Market$puts[,1]

# Time to expiry of call options
mkt_vol[1:nrow(Market$calls),3] <- Market$calls[,2]

# Time to expiry of put options
mkt_vol[(nrow(Market$calls)+1):nb_opts,3] <- Market$puts[,2]

# Implied volatility of call options
mkt_vol[1:nrow(Market$calls),4] <- Market$calls[,3]

# Implied volatility of put options
mkt_vol[(nrow(Market$calls)+1):nb_opts,4] <- Market$puts[,3]
...
x1 <- sp_500[n_obs] * seq(0.5, 1.5, (1.5 - 0.5) / 1000)
...
y  <- vol_surface(sp_500[n_obs], x3[,1], x3[,2], theta_vol[1], theta_vol[2], theta_vol[3], theta_vol[4])
...
Resid_returns <- fit@fit$residuals
